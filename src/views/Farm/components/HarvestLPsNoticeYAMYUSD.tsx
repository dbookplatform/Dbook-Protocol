...
const HarvestLPsNoticeYAMYUSD: React.FC = () => {
  ...
  }, [status, stakedBalanceYAMYUSD, earnedBalanceYAMYUSD]);
  ...
  const HarvestNotice = useMemo(() => {
    if (
      status === "connected" &&
      (stakedBalanceYUSDLP > 0 || earnedBalanceYUSDLP > 0)
    ) {
      return (
        <>
          <Notice>
            ...
            <NoticeContent>
              <StyledNoticeContentInner>
                <span>
                  ...
                  {(stakedBalanceYUSDLP
                    ? stakedBalanceYUSDLP + " LP tokens"
                    : "") +
                    (stakedBalanceYUSDLP && earnedBalanceYUSDLP
                      ? " and you have "
                      : "") +
                    (earnedBalanceYUSDLP
                      ? earnedBalanceYUSDLP.toFixed(2) + " DBOOK!"
                      : "")}{" "}
                </span>
                <Box flex={1} />
                <Spacer size="sm" />
                <span>
                  <Button
                    ...
                    text={
                      earnedBalanceYUSDLP
                        ? "Harvest DBOOK from Old LP"
                        : "Harvest & Unstake DBOOK/yUSD"
                    }
                    onClick={
                      !stakedBalanceYUSDLP && earnedBalanceYUSDLP
                        ? onHarvestYAMYUSD
  ...
  }, [status, stakedBalanceYUSDLP, earnedBalanceYUSDLP, onHarvestYAMYUSD, onRedeemYAMYUSD]);
