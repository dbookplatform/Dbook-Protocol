...
const Treasury: React.FC = () => {
  ...
  const assetYUSD =
    (totalYUsdValue ? totalYUsdValue : 0) * (yusdPrice ? yusdPrice : 0);
  const assetDPI =
    (totalDPIValue ? totalDPIValue : 0) * (dpiPrice ? dpiPrice : 0);
  const assetWETH =
    (totalWETHValue ? totalWETHValue : 0) * (wethPrice ? wethPrice : 0);
  ...
  const treasuryAssets =
    assetYUSD + assetDPI + assetWETH + assetIndexLP + assetIndex + totalSushi;
  const treasuryValue =
    ...
    <Card>
      ...
      <CardContent>
        <Split>
          ...
          <FancyValue icon="🍠" label="DBOOK in reserves" value={yamValue} />
        </Split>
        <Spacer />
      </CardContent>
      <CardActions>
