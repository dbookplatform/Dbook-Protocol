    ...
    AccessControlUpgradeable
{
    ...
    function initialize(
        uint256 _lockTimeYears,
        uint256 _vestTimeYears,
        address _DBK
    ...
    function addTeamFund(uint256 amount) external onlyTeamAdmin {
    ...
    function addTeamMember(address payable _teamMember, uint256 _amount)
        external
        onlyTeamAdmin
    {
        require(
            _amount <= amountUnallocated,
            "DBKTeamMember: Allocating more funds that the remaiing unallocated amount!"
        );

        uint256 rewardRate = _amount.div(vestTime); //unique token/s

        teamMembers[_teamMember] = TeamMemberStake(
            block.timestamp,
            _amount,
            rewardRate,
            0
        ...
        _setupRole(TEAM_MEMBER_ROLE, _teamMember);
    ...
    function claimTokens() external onlyTeamMember nonReentrant {
        ...
        teamMembers[_msgSender()].claimed = (teamMembers[_msgSender()].claimed)
            .add(currentReward);
    ...
    function calculateReward(address _member) public view returns (uint256) {
        require(
            teamMemberExist[_member],
            "DBKTeamContract: This member does not exist!"
        );
        uint256 timePassed = block.timestamp.sub(teamMembers[_member].start);

        require(
            timePassed >= lockTime,
            "DBKTeamMember: Funds have not been locked for the minimum amount of time!"
        );
        ...
        return
            ((teamMembers[_member].rewardRate).mul(timePassed.sub(lockTime)))
                .sub(teamMembers[_member].claimed);
    }
    ...
    modifier onlyTeamAdmin() {
        require(
            hasRole(TEAM_ADMIN_ROLE, _msgSender()),
            "DBKTeamContract: must have team admin role to use this function"
        );
        _;
    }
    ...
    modifier onlyTeamMember() {
        require(
            hasRole(TEAM_MEMBER_ROLE, _msgSender()),
            "DBKTeamContract: must have team member role to use this function"
        );
        _;
    }
