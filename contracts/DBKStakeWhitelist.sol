...
contract DBKStake is ReentrancyGuardUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    mapping(uint256 => bool) public _whitelist;
    ...
    function seedFirstPool(
        uint256 _poolSeedAmount,
        uint256 _poolSeedStartTime
    ) public onlyAdmin{
        require(!init_first_pool_flag, "DBKStake: First Pool has already been seeded!");
        
        currentCycle = 1;

        _poolTimes[currentCycle] = PoolTimes({start: _poolSeedStartTime, end: _poolSeedStartTime.add(_durationForCycle[1])});
    ...
    function stake(uint256 amount) public nonReentrant whenNotPaused {
        ...
        require(block.timestamp <= _poolTimes[currentCycle].start + uint256(2).mul(duration), "DBKStake: Too late to deposit, wait for next cycle!");
        ...
        if (!_userStakedOnCycle[currentCycle][_msgSender()]){
            ...
            _stakeOnCycle[currentCycle][msg.sender].amountStaked = _stakeOnCycle[currentCycle][msg.sender].amountStaked.add(amount);
        }
    ...
    function unstake(uint256 _cycle) public nonReentrant whenNotPaused {
        require(_userStakedRequirements(_msgSender(), _cycle), "DBKStake: User Does Not Meet the unstake Requirements!");
    ...
    function adminDepositForNextPool(address rewardToken, uint256 amount , uint256 _stakeOffset, uint256 _days) public onlyAdmin {
        require(amount != 0, "Amount Cannot be Zero");
        require(!_adminPoolDepositFlag[currentCycle+1], "DBKStake: Deposit has already been made for the upcoming cycle!");
        ...
        _poolTimes[currentCycle+1] = PoolTimes({start: (_poolTimes[currentCycle].end).add(_stakeOffset), end: (_poolTimes[currentCycle].end).add(_stakeOffset) + _days.mul(duration) });
        ...
        emit nextPoolUpdated(currentCycle.add(1), (_poolTimes[currentCycle].end).add(_stakeOffset), amount, _days.mul(duration));
    }
    ...
    function updateWhitelist(address _address, bool _flag) public onlyAdmin {
     ...
     */
    function adminRescueTokens(address token, address recipient, uint256 amount) public onlyAdmin {
        ...
        if(token == address(_rewardTokenForCycle[currentCycle])){
            ...
            require(amount < IERC20Upgradeable(token).balanceOf(address(this)).sub(_totalRewardSupplyOnCycle[currentCycle]), "DBKStake: Amount attmepting to withdraw exceeds the allocated awards!");
        }
    ...
    function _userStakedRequirements(address _user, uint256 _cycle) internal view returns(bool){
        ...
        require(block.timestamp.sub(_stakeOnCycle[_cycle][_user].timeStaked) > 1 days, "24 hour staked requirement has not been met!");
        return true;
    }
    ...
    function _calculateReward(uint256 _cycle, address _user) internal view returns(uint256){
        ...
        return (((_stake.amountStaked.mul(_baseReward[_cycle]).mul(multiplier)).mul(durationStaked(_cycle,_user)))).div(_totalStakingSupplyOnCycle[_cycle]);
    }
    ...
    function durationStaked(uint256 _cycle, address _user) public view returns(uint256){
        return (MathUpgradeable.min(block.timestamp, _poolTimes[_cycle].end)).sub(_stakeOnCycle[_cycle][_user].timeStaked);
    }
