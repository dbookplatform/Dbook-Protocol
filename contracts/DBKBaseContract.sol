    ...
    AccessControlUpgradeable
{
     ...
     */
    function withdrawFunds(address _tokenToWithdraw, uint256 _amount)
        external
        onlyBaseAdmin
    {
        ...
        require(
            block.timestamp >= lastWithdraw.add(withdrawLength),
            "DBKBaseContract: Time has not advanced enough since the last withdraw!"
        );
        require(
            checkLoss(),
            "DBKBaseContract: No Loss for the operation found!"
        );
        require(
            token.balanceOf(address(this)) > 100000,
            "DBKBaseContract: Insufficient balance for this token!"
        );
        require(
            _amount <=
                (token.balanceOf(address(this)).mul(percentageCap)).div(
                    percentageDivisor
                ),
            "DBKBaseContract: Attempting to withdraw more than the percentage allowed"
        );
    ...
    function changePercentageCap(uint256 _percentageCap)
        external
        onlyController
    {
        require(
            percentageCap <= percentageDivisor,
            "DBKBaseContract: Percentage given is too large!"
        );
    ...
    function changeWithdrawLength(uint256 _days) external onlyController {
    ...
    function addController(address _address) external onlyDefaultAdmin {
        _setupRole(CONTROLLER_ROLE, _address);
        emit controllerAdded(_address);
    }

    function addBaseAdmin(address _address) external onlyDefaultAdmin {
    ...
    modifier onlyBaseAdmin() {
        require(
            hasRole(BASE_ADMIN_ROLE, _msgSender()),
            "DBKBaseContract: must have role to use this function"
        );
        _;
    }
